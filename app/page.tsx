...
// กำหนด Type ข้อมูล Pokemon จาก PokeAPI
...
// Normalized Pokemon type for components
interface Pokemon {
  id: string;
  name: string;
  image: string;
...
function transformPokemon(pokemon: PokeAPIPokemon): Pokemon {
  const sprites = pokemon.pokemon_v2_pokemonsprites[0]?.sprites || null;
  return {
    id: pokemon.id.toString(),
    name: capitalize(pokemon.name),
    image: getPokemonImageFromSprites(sprites, pokemon.id),
...
function SearchPageContent() {
  ...
  const fetchPokemons = useCallback(async (offset: number = 0, append: boolean = false) => {
    if (append) {
  ...
  }, [fetchPokemons, hasMore, isLoadingMore, loading]);
